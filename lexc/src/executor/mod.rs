...
use std::sync::Arc;
use base64::Engine;
use crate::executor::llm_adapter::{ProviderKind, ProviderConfig};
...
impl ExecutionEnvironment {
    ...
    pub fn new(config: ExecutorConfig) -> Self {
        ...
        Self {
            ...
            session_ttl_ms: std::env::var("LEXON_SESSION_TTL_MS").ok().and_then(|v| v.parse().ok()).unwrap_or(10*60*1000),
            session_gc_interval_ms: std::env::var("LEXON_SESSION_GC_INTERVAL_MS").ok().and_then(|v| v.parse().ok()).unwrap_or(60*1000),
            last_session_gc_epoch_ms: 0,
            quality_schema_enforce: std::env::var("LEXON_QUALITY_SCHEMA_ENFORCE").ok().map(|v| v.to_lowercase()=="true").unwrap_or(false),
            quality_pii_block: std::env::var("LEXON_QUALITY_PII_BLOCK").ok().map(|v| v.to_lowercase()=="true").unwrap_or(false),
        }
    }

    fn now_epoch_ms() -> u64 {
        std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or(std::time::Duration::from_millis(0)).as_millis() as u64
    }
        ...
        if let Some(meta) = self.tool_registry.get_mut(name) {
            if let Some(limit) = meta.allowed_calls {
                if meta.used_calls >= limit {
                    return Err(ExecutorError::RuntimeError(format!("Tool '{}' quota exceeded", name)));
                }
            }
    ...
    fn after_tool_event(&mut self, name: &str, success: bool, error: Option<&str>) {
        if success { println!("✅ on_tool_success: {}", name); }
        else { println!("❌ on_tool_error: {} - {}", name, error.unwrap_or("unknown")); }
    }
    ...
    fn record_llm_call(&mut self, elapsed_ms: u128) {
        ...
        println!("[INFO] [llm_call] COMPLETED in {:?}", std::time::Duration::from_millis(elapsed_ms as u64));
    }
        ...
        for line in lower.lines() {
            if let Some(pos) = line.find("score") {
                ...
                for tok in tail.split(|c: char| !c.is_ascii_digit() && c != '.') {
                    ...
                    if let Ok(v) = tok.parse::<f32>() { return v.clamp(0.0, 1.0); }
                }
            }
        }
        0.0
    ...
    fn encode_tokens_precise(text: &str, model: Option<&str>) -> Option<(tiktoken_rs::CoreBPE, Vec<usize>)> {
        let bpe = if let Some(m) = model {
            tiktoken_rs::get_bpe_from_model(m).ok().or_else(|| tiktoken_rs::cl100k_base().ok())
        } else {
    ...
    #[cfg(not(feature = "tokenizers"))]
    fn tokens_to_strings(_bpe: &(), _ids: &[usize]) -> Vec<String> { Vec::new() }
    ...
    #[cfg(not(feature = "tokenizers"))]
    fn decode_range_to_string(_bpe: &(), _ids: &[usize]) -> String { String::new() }
    ...
    fn save_prompts(map: &serde_json::Map<String, serde_json::Value>) -> Result<()> {
        ...
        std::fs::write(&path, serde_json::to_string_pretty(&serde_json::Value::Object(map.clone())).unwrap_or_else(|_| "{}".to_string()))
    ...
    fn distcache_path() -> String {
        std::env::var("LEXON_DISTCACHE_PATH").unwrap_or_else(|_| "samples/cache/dist_cache.json".to_string())
    }
    ...
    fn execute_tool_call_json(&mut self, name: &str, args: &serde_json::Value) -> Result<serde_json::Value> {
        let out_ref = ValueRef::Named("__mcp_out".to_string());
        match name {
            "read_file" => {
                let path = args.get("path").and_then(|v| v.as_str()).or_else(|| args.as_str()).ok_or_else(|| ExecutorError::ArgumentError("read_file requires 'path'".to_string()))?.to_string();
                let arg_exprs = vec![LexExpression::Value(ValueRef::Literal(LexLiteral::String(path)))];
            ...
            "emit_file" => {
                ...
                let (name, mime, b64) = if let Some(arr) = args.as_array() {
                    let n = arr.get(0).and_then(|v| v.as_str()).unwrap_or("").to_string();
                    let m = arr.get(1).and_then(|v| v.as_str()).unwrap_or("application/octet-stream").to_string();
                    let b = arr.get(2).and_then(|v| v.as_str()).unwrap_or("").to_string();
                    (n,m,b)
                } else {
                    let n = args.get("name").and_then(|v| v.as_str()).unwrap_or("").to_string();
                    let m = args.get("mime").and_then(|v| v.as_str()).unwrap_or("application/octet-stream").to_string();
                    let b = args.get("base64").and_then(|v| v.as_str()).unwrap_or("").to_string();
                    (n,m,b)
                };
            ...
            "save_binary_file" => {
                ...
                let (path, b64) = if let Some(arr) = args.as_array() {
                    let p = arr.get(0).and_then(|v| v.as_str()).unwrap_or("").to_string();
                    let b = arr.get(1).and_then(|v| v.as_str()).unwrap_or("").to_string();
                    (p,b)
                } else {
                    let p = args.get("path").and_then(|v| v.as_str()).unwrap_or("").to_string();
                    let b = args.get("base64").and_then(|v| v.as_str()).unwrap_or("").to_string();
                    (p,b)
                };
                let bytes = base64::engine::general_purpose::STANDARD
                ...
                let file = BinaryFile::new("file.bin".to_string(), bytes, "application/octet-stream".to_string());
                let mo = self.create_multioutput("".to_string(), vec![file]);
                let arg_exprs = vec![
            ...
            "write_file" => {
                let (path, content) = if let (Some(p), Some(c)) = (args.get("path").and_then(|v| v.as_str()), args.get("content")) {
                    (p.to_string(), if let Some(s) = c.as_str() { s.to_string() } else { c.to_string() })
                } else if let Some(a) = args.as_array() { if a.len()==2 { (a[0].as_str().unwrap_or("").to_string(), a[1].as_str().unwrap_or("").to_string()) } else { return Err(ExecutorError::ArgumentError("write_file expects [path, content]".to_string())); } } else { return Err(ExecutorError::ArgumentError("write_file requires 'path' and 'content'".to_string())); };
                let arg_exprs = vec![
                    LexExpression::Value(ValueRef::Literal(LexLiteral::String(path))),
            ...
            "save_file" => {
                let (content, path) = if let (Some(c), Some(p)) = (args.get("content"), args.get("path").and_then(|v| v.as_str())) {
                    (if let Some(s) = c.as_str() { s.to_string() } else { c.to_string() }, p.to_string())
                } else { return Err(ExecutorError::ArgumentError("save_file requires 'content' and 'path'".to_string())); };
                let arg_exprs = vec![
                    LexExpression::Value(ValueRef::Literal(LexLiteral::String(content))),
                    LexExpression::Value(ValueRef::Literal(LexLiteral::String(path)))
                ];
            ...
            "load_file" => {
                let path = args.get("path").and_then(|v| v.as_str()).or_else(|| args.as_str()).ok_or_else(|| ExecutorError::ArgumentError("load_file requires 'path'".to_string()))?.to_string();
                let arg_exprs = vec![LexExpression::Value(ValueRef::Literal(LexLiteral::String(path)))];
                self.handle_load_file(&arg_exprs, Some(&out_ref))?;
            }
            "load_binary_file" => {
                let path = args.get("path").and_then(|v| v.as_str()).or_else(|| args.as_str()).ok_or_else(|| ExecutorError::ArgumentError("load_binary_file requires 'path'".to_string()))?.to_string();
                let mut arg_exprs = vec![LexExpression::Value(ValueRef::Literal(LexLiteral::String(path)))];
                if let Some(n) = args.get("name").and_then(|v| v.as_str()) { arg_exprs.push(LexExpression::Value(ValueRef::Literal(LexLiteral::String(n.to_string())))); }
                self.handle_load_binary_file(&arg_exprs, Some(&out_ref))?;
            }
            other => return Err(ExecutorError::UndefinedFunction(other.to_string())),
        }
        let val = self.variables.get("__mcp_out").cloned().unwrap_or(RuntimeValue::Null);
        Ok(match val { RuntimeValue::Json(j)=>j, RuntimeValue::String(s)=>serde_json::json!(s), RuntimeValue::Boolean(b)=>serde_json::json!(b), RuntimeValue::Integer(i)=>serde_json::json!(i), RuntimeValue::Float(f)=>serde_json::json!(f), RuntimeValue::Null=>serde_json::Value::Null, _=>serde_json::json!(format!("{:?}", val)) })
    }
        ...
        match instruction {
            ...
            LexInstruction::Call {
                ...
                if function == "print" {
                    ...
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("channel.create requires name".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, v=>format!("{:?}", v) };
                    let cap: usize = if args.len()>1 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=> (i as isize).max(1) as usize, RuntimeValue::Float(f)=> (f as isize).max(1) as usize, RuntimeValue::String(s)=> s.parse().unwrap_or(32), _=>32 } } else { 32 };
                    ...
                    if args.len()<2 { return Err(ExecutorError::ArgumentError("channel.send requires name and value".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, v=>format!("{:?}", v) };
                    let val_v = self.evaluate_expression(args[1].clone())?;
                    let payload = match val_v { RuntimeValue::String(s)=>s, RuntimeValue::Json(j)=> j.to_string(), other=> format!("{:?}", other) };
                    let ok = if let Some(tx) = self.channel_senders.get(&name) {
                        let txc = tx.clone();
                        tokio::task::block_in_place(|| tokio::runtime::Handle::current().block_on(async move { txc.send(payload).await.is_ok() }))
                    } else { false };
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(ok))?; }
                    Ok(())
                } else if function == "channel.recv" || function == "channel__recv" {
                    // channel.recv(name, [timeout_ms]) -> string
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("channel.recv requires name".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, v=>format!("{:?}", v) };
                    let timeout_ms: Option<u64> = if args.len()>1 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=>Some(i as u64), RuntimeValue::Float(f)=>Some(f as u64), RuntimeValue::String(s)=> s.parse().ok(), _=>None } } else { None };
                    let rx = self.channel_receivers.get_mut(&name).ok_or_else(|| ExecutorError::RuntimeError(format!("channel '{}' not found", name)))?;
                    let res = tokio::task::block_in_place(|| tokio::runtime::Handle::current().block_on(async {
                        if let Some(ms)=timeout_ms { tokio::time::timeout(std::time::Duration::from_millis(ms), rx.recv()).await.ok().flatten() } else { rx.recv().await }
                    }));
                    match res {
                        Some(s) => { if let Some(r)=result { self.store_value(r, RuntimeValue::String(s))?; } Ok(()) },
                        None => Err(ExecutorError::RuntimeError("channel.recv timeout or closed".to_string())),
                    }
                } else if function == "channel.select_any" || function == "channel__select_any" {
                    // channel.select_any([names], [timeout_ms]) -> {name,value}
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("channel.select_any requires names array".to_string())); }
                    let names_v = self.evaluate_expression(args[0].clone())?;
                    let mut names: Vec<String> = match names_v {
                        ...
                        _ => return Err(ExecutorError::ArgumentError("names must be array or string".to_string())),
                    };
                    if names.is_empty() { return Err(ExecutorError::ArgumentError("no channel names provided".to_string())); }
                    let timeout_ms: u64 = if args.len()>1 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=> (i as i64).max(0) as u64, RuntimeValue::Float(f)=> (f as i64).max(0) as u64, RuntimeValue::String(s)=> s.parse().unwrap_or(1000), _=>1000 } } else { 1000 };
                    ...
                    loop {
                            ...
                            if let Some(rx) = self.channel_receivers.get_mut(name) {
                                match rx.try_recv() {
                                    Ok(val) => {
                                        ...
                                        if let Some(r)=result { self.store_value(r, RuntimeValue::Json(obj))?; }
                                        return Ok(());
                                    }
                        ...
                        if start.elapsed().as_millis() as u64 >= timeout_ms { return Err(ExecutorError::RuntimeError("channel.select_any timeout".to_string())); }
                        tokio::task::block_in_place(|| tokio::runtime::Handle::current().block_on(async { tokio::time::sleep(poll_interval).await }));
                    }
                } else if function == "task.spawn" || function == "task__spawn" {
                    // task.spawn(prompt[, model]) -> task_id
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("task.spawn requires prompt".to_string())); }
                    ...
                    let scheduler = self.async_scheduler.as_ref().ok_or_else(|| ExecutorError::RuntimeError("scheduler unavailable".to_string()))?;
                    let (task_id, handle) = tokio::task::block_in_place(|| {
                        tokio::runtime::Handle::current().block_on(async {
                            let mut adapter_seed = self.llm_adapter.clone();
                            let m = model_opt.clone();
                            let h = scheduler.schedule_task(
                                move |_token| {
                                    let mut adapter = adapter_seed.clone();
                                    let p = prompt.clone();
                                    async move {
                                        adapter.call_llm_async(m.as_deref(), Some(0.7), None, Some(&p), None, None, &std::collections::HashMap::new()).await
                                            .map_err(|e| format!("{}", e))
                                    }
                                },
                                crate::runtime::scheduler::TaskPriority::Normal,
                                None,
                            ).await.map_err(|e| ExecutorError::RuntimeError(e))?;
                            let id = h.id().to_string();
                            Ok::<(String, crate::runtime::scheduler::TaskHandle), ExecutorError>((id, h))
                        })
                    })?;
                    self.tasks.insert(task_id.clone(), handle);
                    if let Some(res)=result { self.store_value(res, RuntimeValue::String(task_id))?; }
                    Ok(())
                } else if function == "task.await" || function == "task__await" {
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("task.await requires id".to_string())); }
                    let id = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let timeout_ms = if args.len()>1 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=>Some(i as u64), RuntimeValue::Float(f)=>Some(f as u64), RuntimeValue::String(s)=>s.parse().ok(), _=>None } } else { None };
                    let handle = self.tasks.get(&id).cloned().ok_or_else(|| ExecutorError::RuntimeError(format!("task '{}' not found", id)))?;
                    let res = tokio::task::block_in_place(|| { tokio::runtime::Handle::current().block_on(async {
                        if let Some(ms)=timeout_ms { handle.await_result_timeout(std::time::Duration::from_millis(ms)).await } else { handle.await_result().await }
                    })});
                    use crate::runtime::scheduler::TaskResult;
                    match res {
                        Ok(TaskResult::Success(s)) => { if let Some(r)=result { self.store_value(r, RuntimeValue::String(s))?; } }
                        Ok(TaskResult::Error(e)) => return Err(ExecutorError::RuntimeError(e)),
                        Ok(TaskResult::Cancelled(reason)) => return Err(ExecutorError::RuntimeError(format!("cancelled: {}", reason))),
                        Ok(TaskResult::Timeout) => return Err(ExecutorError::RuntimeError("task.await timeout".to_string())),
                        Err(e) => return Err(ExecutorError::RuntimeError(format!("await error: {}", e))),
                    }
                    // Optional: remove handle after await completes
                    // let _ = self.tasks.remove(&id);
                    Ok(())
                } else if function == "task.cancel" || function == "task__cancel" {
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("task.cancel requires id".to_string())); }
                    let id = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    if let Some(handle) = self.tasks.get(&id) {
                        let ok = tokio::task::block_in_place(|| { tokio::runtime::Handle::current().block_on(async { handle.cancel().await.is_ok() }) });
                        if let Some(r)=result { self.store_value(r, RuntimeValue::Boolean(ok))?; }
                        Ok(())
                    } else {
                        return Err(ExecutorError::RuntimeError(format!("task '{}' not found", id)))
                    }
                    ...
                    if args.len() < 1 { return Err(ExecutorError::ArgumentError("session.configure requires config json".to_string())); }
                    let cfg = match self.evaluate_expression(args[0].clone())? { RuntimeValue::Json(j)=>j, RuntimeValue::String(s)=> serde_json::from_str(&s).unwrap_or(serde_json::Value::Null), _=> serde_json::Value::Null };
                    ...
                    if args.is_empty() {
                        ...
                        for (k,v) in &self.agent_status { map.insert(k.clone(), serde_json::Value::String(v.clone())); }
                        if let Some(res)=result { self.store_value(res, RuntimeValue::Json(serde_json::Value::Object(map)))?; }
                    } else {
                        let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, v=>format!("{:?}", v) };
                        let st = self.agent_status.get(&name).cloned().unwrap_or_else(|| "unknown".to_string());
                        if let Some(res)=result { self.store_value(res, RuntimeValue::String(st))?; }
                    }
                    Ok(())
                } else if function == "agent_list" {
                    let names: Vec<serde_json::Value> = self.agent_registry.keys().cloned().map(serde_json::Value::String).collect();
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(serde_json::Value::Array(names)))?; }
                    Ok(())
                } else if function == "agent_supervisor.get_state" || function == "agent_supervisor__get_state" {
                    if args.len()!=1 { return Err(ExecutorError::ArgumentError("agent_supervisor.get_state requires name".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, v=>format!("{:?}", v) };
                    let cfg = self.agent_registry.get(&name).cloned();
                    let status = self.agent_status.get(&name).cloned().unwrap_or_else(|| "unknown".to_string());
                    let cancelled = *self.agent_cancelled.get(&name).unwrap_or(&false);
                    let obj = if let Some(c)=cfg { serde_json::json!({"name": name, "model": c.model, "budget_usd": c.budget_usd, "deadline_ms": c.deadline_ms, "status": status, "cancelled": cancelled}) } else { serde_json::json!({"name": name, "status": status, "cancelled": cancelled}) };
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(obj))?; }
                    Ok(())
                } else if function == "agent_supervisor.list" || function == "agent_supervisor__list" {
                    let mut arr = Vec::new();
                    ...
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(serde_json::Value::Array(arr)))?; }
                    Ok(())
                    ...
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("memory_index.search requires query,k".to_string())); }
                    let query = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("query must be string".to_string())) };
                    let k = match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=>i as usize, _=>return Err(ExecutorError::ArgumentError("k must be integer".to_string())) };
                    let results = if let Some(ref mut vm)=self.vector_memory_system { vm.vector_search(&query,k)? } else { Vec::new() };
                    let arr: Vec<Value> = results.into_iter().map(|v| match v { RuntimeValue::Json(j)=>j, RuntimeValue::String(s)=>Value::String(s), other=>Value::String(format!("{:?}",other)) }).collect();
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(Value::Array(arr)))?; }
                    Ok(())
                } else if function == "memory_index.hybrid_search" || function == "memory_index__hybrid_search" {
                    if args.len()<2 { return Err(ExecutorError::ArgumentError("hybrid_search requires query,k,[config]".to_string())); }
                    let query = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("query must be string".to_string())) };
                    let k = match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=>i as usize, _=>return Err(ExecutorError::ArgumentError("k must be integer".to_string())) };
                    let mut alpha: f32 = 0.7; let mut filters: HashMap<String,String> = HashMap::new(); let mut offset: usize = 0; let mut limit_factor: usize = 4;
                    if args.len()>=3 {
                        match self.evaluate_expression(args[2].clone())? {
                            ...
                            RuntimeValue::String(s)=>{ if let Ok(v)=s.parse::<f32>(){ alpha=v.clamp(0.0,1.0);} }
                            RuntimeValue::Json(Value::Object(map))=>{
                                if let Some(a)=map.get("alpha").and_then(|v| v.as_f64()){ alpha=(a as f32).clamp(0.0,1.0);} 
                                if let Some(fm)=map.get("filters").and_then(|v| v.as_object()){ for (k,v) in fm { if let Some(s)=v.as_str(){ filters.insert(k.clone(), s.to_string()); } else { filters.insert(k.clone(), v.to_string()); } } }
                                if let Some(raw)=map.get("qdrant_filter") { filters.insert("__raw__".to_string(), raw.to_string()); }
                                if let Some(o)=map.get("offset").and_then(|v| v.as_u64()) { offset = o as usize; }
                                if let Some(lf)=map.get("limit_factor").and_then(|v| v.as_u64()) { let lf_usize = lf as usize; if lf_usize>=1 { limit_factor = lf_usize; } }
                            }
                            _=>{}
                        }
                    }
                    let results = if let Some(ref mut vm)=self.vector_memory_system { vm.hybrid_search(&query,k,alpha,&filters, offset, limit_factor)? } else { Vec::new() };
                    let arr: Vec<Value> = results.into_iter().map(|v| match v { RuntimeValue::Json(j)=>j, RuntimeValue::String(s)=>Value::String(s), other=>Value::String(format!("{:?}",other)) }).collect();
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(Value::Array(arr)))?; }
                    Ok(())
                } else if function == "memory_index.hybrid_search_page" || function == "memory_index__hybrid_search_page" {
                    // memory_index.hybrid_search_page(query, k, page, limit_factor?, config?)
                    if args.len()<3 { return Err(ExecutorError::ArgumentError("hybrid_search_page requires query,k,page".to_string())); }
                    let query = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("query must be string".to_string())) };
                    let k = match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=>i as usize, _=>return Err(ExecutorError::ArgumentError("k must be integer".to_string())) };
                    let page = match self.evaluate_expression(args[2].clone())? { RuntimeValue::Integer(i)=> if i<0 {0usize} else { i as usize }, _=>0usize };
                    let mut limit_factor: usize = if args.len()>3 { match self.evaluate_expression(args[3].clone())? { RuntimeValue::Integer(i)=> (i as usize).max(1), _=>4 } } else { 4 };
                    let mut alpha: f32 = 0.7; let mut filters: HashMap<String,String> = HashMap::new();
                    if args.len()>4 {
                    ...
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(Value::Array(arr)))?; }
                    Ok(())
                } else if function == "memory_index.hybrid_search_all" || function == "memory_index__hybrid_search_all" {
                    // memory_index.hybrid_search_all(query, k, max_pages?, limit_factor?, config?) -> results_json
                    if args.len()<2 { return Err(ExecutorError::ArgumentError("hybrid_search_all requires query,k".to_string())); }
                    let query = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("query must be string".to_string())) };
                    let k = match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=>i as usize, _=>return Err(ExecutorError::ArgumentError("k must be integer".to_string())) };
                    let max_pages: usize = if args.len()>2 { match self.evaluate_expression(args[2].clone())? { RuntimeValue::Integer(i)=> (i as usize).max(1), _=> 5 } } else { 5 };
                    let mut limit_factor: usize = if args.len()>3 { match self.evaluate_expression(args[3].clone())? { RuntimeValue::Integer(i)=> (i as usize).max(1), _=>4 } } else { 4 };
                    let mut alpha: f32 = 0.7; let mut filters: HashMap<String,String> = HashMap::new();
                    if args.len()>4 {
                        match self.evaluate_expression(args[4].clone())? {
                    ...
                    if let Some(ref mut vm)=self.vector_memory_system {
                        for page in 0..max_pages {
                            let offset = page.saturating_mul(k);
                            let mut page_res = vm.hybrid_search(&query, k, alpha, &filters, offset, limit_factor)?;
                            if page_res.is_empty() { break; }
                    ...
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(Value::Array(out)))?; }
                    Ok(())
                } else if function == "memory_index.hybrid_search_llm_rerank" || function == "memory_index__hybrid_search_llm_rerank" {
                    // memory_index.hybrid_search_llm_rerank(query, k, config?, model?, top_k?)
                    if args.len()<2 { return Err(ExecutorError::ArgumentError("hybrid_search_llm_rerank requires query,k".to_string())); }
                    let query = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("query must be string".to_string())) };
                    let k = match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=>i as usize, _=>return Err(ExecutorError::ArgumentError("k must be integer".to_string())) };
                    let mut alpha: f32 = 0.7; let mut filters: HashMap<String,String> = HashMap::new();
                    let mut model: Option<String> = None; let mut top_k: usize = k;
                    if args.len()>2 {
                        match self.evaluate_expression(args[2].clone())? {
                            RuntimeValue::Json(Value::Object(map))=>{
                                if let Some(a)=map.get("alpha").and_then(|v| v.as_f64()){ alpha=(a as f32).clamp(0.0,1.0);} 
                    ...
                    if args.len()>3 { if let RuntimeValue::String(s) = self.evaluate_expression(args[3].clone())? { model = Some(s); } }
                    if args.len()>4 { if let RuntimeValue::Integer(i) = self.evaluate_expression(args[4].clone())? { top_k = (i as usize).max(1); } }
                    let mut arr: Vec<Value> = Vec::new();
                    if let Some(ref mut vm)=self.vector_memory_system {
                        let res = vm.hybrid_search(&query, k, alpha, &filters, 0, 4)?;
                        for v in res { match v { RuntimeValue::Json(j)=>arr.push(j), RuntimeValue::String(s)=>arr.push(Value::String(s)), other=>arr.push(Value::String(format!("{:?}",other))) } }
                    }
                    ...
                    if let Some(ref mut la) = self.llm_adapter_new {
                        ...
                        let scores: Vec<f32> = tokio::task::block_in_place(|| handle.block_on(async {
                        ...
                        for (i, it) in arr.into_iter().enumerate() { let sc = scores.get(i).cloned().unwrap_or(0.0); scored.push((sc, it)); }
                    } else {
                        for it in arr.into_iter() {
                            let content = it.get("content").and_then(|v| v.as_str()).unwrap_or("");
                            let prompt = format!("Given query: {:?}\nRate relevance of passage 0..1 (decimal).\nPassage: {:?}\nAnswer as: score: <number>", query, content);
                            let resp = self.llm_adapter.call_llm(model.as_deref(), Some(0.0), Some(sys), Some(&prompt), None, None, &HashMap::new())?;
                    ...
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(out))?; }
                    Ok(())
                } else if function == "memory_index.set_metadata" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("set_metadata requires path, metadata".to_string())); }
                    let path = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("path must be string".to_string())) };
                            ...
                            match serde_json::from_str::<Value>(&s) {
                                ...
                                Err(_)=>{
                                    ...
                                    if let Some((k,v)) = trimmed.split_once(':') {
                                        map.insert(k.trim().trim_matches('"').to_string(), serde_json::Value::String(v.trim().trim_matches('"').to_string()));
                                    }
                                    Value::Object(map)
                                }
                            }
                        }
                        _=>return Err(ExecutorError::ArgumentError("metadata must be JSON".to_string()))
                    };
                    if let Some(ref mut vm)=self.vector_memory_system { vm.set_metadata(&path, &metadata)?; }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                } else if function == "memory_index.qdrant_create_index" || function == "memory_index__qdrant_create_index" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("qdrant_create_index requires field_name, field_schema".to_string())); }
                    let field = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=> return Err(ExecutorError::ArgumentError("field_name must be string".to_string())) };
                    let schema = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, _=> return Err(ExecutorError::ArgumentError("field_schema must be string".to_string())) };
                    if let Some(ref vm) = self.vector_memory_system { vm.qdrant_create_index(&field, &schema)?; }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                } else if function == "memory_index.qdrant_ensure_schema" || function == "memory_index__qdrant_ensure_schema" {
                    if args.len()!=1 { return Err(ExecutorError::ArgumentError("qdrant_ensure_schema requires schema_json".to_string())); }
                    let val = self.evaluate_expression(args[0].clone())?;
                    let schema = match val {
                        RuntimeValue::Json(j)=>j,
                        RuntimeValue::String(s)=> serde_json::from_str::<Value>(&s).map_err(|e| ExecutorError::ArgumentError(format!("Invalid JSON: {}", e)))?,
                        _=> return Err(ExecutorError::ArgumentError("schema must be JSON or string".to_string()))
                    };
                    if let Some(ref vm) = self.vector_memory_system { vm.qdrant_set_schema(&schema)?; }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                    ...
                    if let Some(res) = result { self.store_value(res, RuntimeValue::Json(json))?; }
                    Ok(())
                    ...
                    if args.len() != 1 { return Err(ExecutorError::ArgumentError("metrics.write_json requires exactly 1 argument: path".to_string())); }
                    let path_val = self.evaluate_expression(args[0].clone())?;
                    let path = match path_val { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let json = serde_json::json!({
                        "total_llm_calls": self.total_llm_calls,
                        "llm_total_ms": self.llm_total_ms,
                    });
                    let arg_exprs = vec![
                        LexExpression::Value(ValueRef::Literal(LexLiteral::String(json.to_string()))),
                        LexExpression::Value(ValueRef::Literal(LexLiteral::String(path)))
                    ];
                    self.handle_save_file(&arg_exprs, None)?;
                    if let Some(res) = result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    ...
                    if args.len() < 2 { return Err(ExecutorError::ArgumentError("fixtures.save requires name and content".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let content_val = self.evaluate_expression(args[1].clone())?;
                    let content = match content_val { RuntimeValue::String(s)=>s, RuntimeValue::Json(j)=>j.to_string(), other=>format!("{:?}", other) };
                    let base = std::env::var("LEXON_FIXTURES_DIR").unwrap_or_else(|_| "samples/fixtures".to_string());
                    ...
                    if args.len() < 1 { return Err(ExecutorError::ArgumentError("fixtures.load requires name".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let base = std::env::var("LEXON_FIXTURES_DIR").unwrap_or_else(|_| "samples/fixtures".to_string());
                    let path = format!("{}/{}", base, name);
                    let arg_exprs = vec![ LexExpression::Value(ValueRef::Literal(LexLiteral::String(path))) ];
                    ...
                    if args.len() < 2 { return Err(ExecutorError::ArgumentError("fixtures.save_json requires name and json".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let json_val = self.evaluate_expression(args[1].clone())?;
                    let content = match json_val {
                        RuntimeValue::Json(j)=> serde_json::to_string_pretty(&j).unwrap_or_else(|_| j.to_string()),
                        RuntimeValue::String(s)=> {
                            if let Ok(v) = serde_json::from_str::<Value>(&s) { serde_json::to_string_pretty(&v).unwrap_or(s) } else { s }
                        },
                        other=> format!("{:?}", other)
                    };
                    let base = std::env::var("LEXON_FIXTURES_DIR").unwrap_or_else(|_| "samples/fixtures".to_string());
                    ...
                    if args.len() < 1 { return Err(ExecutorError::ArgumentError("fixtures.load_json requires name".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let base = std::env::var("LEXON_FIXTURES_DIR").unwrap_or_else(|_| "samples/fixtures".to_string());
                    let path = format!("{}/{}", base, name);
                    let txt = std::fs::read_to_string(&path).map_err(|e| ExecutorError::IoError(format!("read: {}", e)))?;
                    ...
                    let mut json: Value = match serde_json::from_str::<Value>(&txt) {
                        ...
                        Err(_) => {
                            ...
                            serde_json::from_str::<Value>(&repaired).map_err(|e| ExecutorError::JsonError(format!("parse: {}", e)))?
                        }
                    };
                    if let Value::String(inner) = &json {
                        if let Ok(v2) = serde_json::from_str::<Value>(inner) { json = v2; }
                    }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(json))?; }
                    Ok(())
                } else if function == "rate_limiter.acquire" || function == "rate_limiter__acquire" {
                    if args.len() < 2 { return Err(ExecutorError::ArgumentError("rate_limiter.acquire requires name and max_per_sec".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let max_ps_val = self.evaluate_expression(args[1].clone())?;
                    let max_per_sec: u32 = match max_ps_val { RuntimeValue::Integer(i)=>i as u32, RuntimeValue::Float(f)=>f as u32, RuntimeValue::String(s)=> s.parse().unwrap_or(1), _=>1 };
                    let now_sec = (std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or(std::time::Duration::from_secs(0)).as_secs()) as u64;
                    let entry = self.rate_limits.entry(name).or_insert((now_sec, 0));
                    if entry.0 != now_sec { entry.0 = now_sec; entry.1 = 0; }
                    let granted = if entry.1 < max_per_sec { entry.1 += 1; true } else { false };
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(granted))?; }
                    Ok(())
                } else if function == "metrics.get_call_logs" || function == "metrics__get_call_logs" {
                    let logs = if let Some(ref a) = self.llm_adapter_new { a.get_call_logs_json() } else { self.llm_adapter.get_call_logs_json() };
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(logs))?; }
                    Ok(())
                } else if function == "metrics.write_call_logs" || function == "metrics__write_call_logs" {
                    if args.len()!=1 { return Err(ExecutorError::ArgumentError("metrics.write_call_logs requires path".to_string())); }
                    let path = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, v=>format!("{:?}", v) };
                    let logs = if let Some(ref a) = self.llm_adapter_new { a.get_call_logs_json() } else { self.llm_adapter.get_call_logs_json() };
                    let arg_exprs = vec![
                        LexExpression::Value(ValueRef::Literal(LexLiteral::String(logs.to_string()))),
                        LexExpression::Value(ValueRef::Literal(LexLiteral::String(path)))
                    ];
                    self.handle_save_file(&arg_exprs, None)?;
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                } else if function == "metrics.export_prometheus" || function == "metrics__export_prometheus" {
                    if args.len()!=1 { return Err(ExecutorError::ArgumentError("metrics.export_prometheus requires path".to_string())); }
                    let path = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, v=>format!("{:?}", v) };
                    let logs = if let Some(ref a) = self.llm_adapter_new { a.get_call_logs_json() } else { self.llm_adapter.get_call_logs_json() };
                    let mut total_cost = 0.0_f64;
                    if let serde_json::Value::Array(arr) = &logs {
                        for e in arr { if let Some(c) = e.get("cost_usd").and_then(|v| v.as_f64()) { total_cost += c; } }
                    }
                    let mut out = String::new();
                    out.push_str("# HELP lexon_llm_calls_total Total LLM calls\n# TYPE lexon_llm_calls_total counter\n");
                    out.push_str(&format!("lexon_llm_calls_total {}\n\n", self.total_llm_calls));

                    out.push_str("# HELP lexon_llm_latency_ms_sum Total LLM latency (ms)\n# TYPE lexon_llm_latency_ms_sum counter\n");
                    out.push_str(&format!("lexon_llm_latency_ms_sum {}\n\n", self.llm_total_ms));
                    ...
                    if let Some(ref a) = self.llm_adapter_new {
                        if let Some(map) = a.routing_metrics_json().get("provider_health").and_then(|v| v.as_object()) {
                            out.push_str("# HELP lexon_provider_health Provider health (1 up, 0 down)\n# TYPE lexon_provider_health gauge\n");
                            for (prov, obj) in map {
                                let up = obj.get("status").and_then(|v| v.as_str()).map(|s| (s!="down") as i32).unwrap_or(1);
                                out.push_str(&format!("lexon_provider_health{{provider=\"{}\"}} {}\n", prov, up));
                            }
                            out.push('\n');
                            wrote_health = true;
                        }
                    }
                    if !wrote_health { out.push_str("# HELP lexon_provider_health Provider health (1 up, 0 down)\n# TYPE lexon_provider_health gauge\n\n"); }
                    ...
                    for (tool, meta) in &self.tool_registry {
                        out.push_str(&format!("lexon_tool_calls_total{{tool=\"{}\"}} {}\n", tool, meta.used_calls));
                    }
                    out.push('\n');

                    out.push_str("# HELP lexon_errors_total Total errors\n# TYPE lexon_errors_total counter\nlexon_errors_total 0\n");

                    std::fs::write(&path, out).map_err(|e| ExecutorError::IoError(format!("metrics.export_prometheus: {}", e)))?;
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                    ...
                    if args.len() < 2 { return Err(ExecutorError::ArgumentError("configure_provider requires name and config".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, v=>format!("{:?}", v) };
                    let cfg_val = self.evaluate_expression(args[1].clone())?;
                    let obj = match cfg_val { RuntimeValue::Json(serde_json::Value::Object(m))=>m, RuntimeValue::String(s)=> serde_json::from_str::<serde_json::Map<String, serde_json::Value>>(&s).unwrap_or_default(), _=> serde_json::Map::new() };
                    let kind = obj.get("kind").and_then(|v| v.as_str()).unwrap_or("custom").to_lowercase();
                    let base_url = obj.get("base_url").and_then(|v| v.as_str()).unwrap_or("").to_string();
                    let api_key = obj.get("api_key").and_then(|v| v.as_str()).map(|s| s.to_string());
                    let p_kind = match kind.as_str() { "huggingface"=> ProviderKind::HuggingFace, "ollama"=> ProviderKind::Ollama, _=> ProviderKind::Custom };
                    let cfg = ProviderConfig { kind: p_kind, base_url, api_key };
                    if let Some(ref mut a) = self.llm_adapter_new { a.register_provider(name.clone(), cfg); } else { self.llm_adapter.register_provider(name.clone(), cfg); }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                } else if function == "providers.override_model" || function == "providers__override_model" {
                    // providers.override_model(model, provider_key)
                    if args.len() < 2 { return Err(ExecutorError::ArgumentError("providers.override_model requires model and provider".to_string())); }
                    let model = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, v=>format!("{:?}", v) };
                    let provider = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, v=>format!("{:?}", v) };
                    if let Some(ref mut a) = self.llm_adapter_new { a.override_model_provider(model.clone(), provider.clone()); } else { self.llm_adapter.override_model_provider(model.clone(), provider.clone()); }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                } else if function == "routing.get_metrics_json" || function == "routing__get_metrics_json" {
                    ...
                    let json = if let Some(ref adapter) = self.llm_adapter_new {
                        ...
                        let is_new_empty = newm.get("ab_variants").map(|v| v.as_object().map(|m| m.is_empty()).unwrap_or(true)).unwrap_or(true)
                            && newm.get("provider_health").map(|v| v.as_object().map(|m| m.is_empty()).unwrap_or(true)).unwrap_or(true);
                        if is_new_empty { legacy } else { newm }
                    } else { legacy };
                    if let Some(res) = result { self.store_value(res, RuntimeValue::Json(json))?; }
                    Ok(())
                    ...
                    if args.len()!=1 { return Err(ExecutorError::ArgumentError("session.exists requires session_id".to_string())); }
                    let sid = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("session_id must be string".to_string())) };
                    let exists = self.memory_manager.session_exists(&sid);
                    if let Some(res) = result { self.store_value(res, RuntimeValue::Boolean(exists))?; }
                    Ok(())
                } else if function == "cache.invalidate" || function == "cache__invalidate" {
                    if args.len() != 1 { return Err(ExecutorError::ArgumentError("cache.invalidate requires key".to_string())); }
                    let key = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("key must be string".to_string())) };
                    let ok = if let Some(ref mut a) = self.llm_adapter_new { a.cache_invalidate(&key) } else { self.llm_adapter.cache_invalidate(&key) };
                    if let Some(res) = result { self.store_value(res, RuntimeValue::Boolean(ok))?; }
                    Ok(())
                } else if function == "cache.invalidate_prefix" || function == "cache__invalidate_prefix" {
                    if args.len() != 1 { return Err(ExecutorError::ArgumentError("cache.invalidate_prefix requires prefix".to_string())); }
                    let prefix = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("prefix must be string".to_string())) };
                    let n = if let Some(ref mut a) = self.llm_adapter_new { a.cache_invalidate_prefix(&prefix) } else { self.llm_adapter.cache_invalidate_prefix(&prefix) } as i64;
                    if let Some(res) = result { self.store_value(res, RuntimeValue::Integer(n))?; }
                    Ok(())
                } else if function == "memory_index.prune_by_metadata" || function == "memory_index__prune_by_metadata" {
                    if args.len()!=1 { return Err(ExecutorError::ArgumentError("prune_by_metadata requires filters object".to_string())); }
                    ...
                    let filters_map: std::collections::HashMap<String, String> = match filters_val {
                        RuntimeValue::Json(serde_json::Value::Object(m)) => m.into_iter().filter_map(|(k,v)| v.as_str().map(|s| (k,s.to_string()))).collect(),
                        RuntimeValue::String(s) => {
                            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&s) {
                                if let Some(m) = v.as_object() { m.iter().filter_map(|(k,v)| v.as_str().map(|s| (k.clone(), s.to_string()))).collect() } else { std::collections::HashMap::new() }
                            } else { std::collections::HashMap::new() }
                        }
                        _ => std::collections::HashMap::new(),
                    };
                    let deleted = if let Some(ref mut v) = self.vector_memory_system { v.prune_by_metadata(&filters_map)? } else { 0 } as i64;
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Integer(deleted))?; }
                    Ok(())
                } else if function == "write_metrics_json" {
                    if args.len()!=1 { return Err(ExecutorError::ArgumentError("write_metrics_json requires path".to_string())); }
                    let path = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("path must be string".to_string())) };
                    // Pull per-call logs from the active adapter (new preferred)
                    let logs_val = if let Some(ref a) = self.llm_adapter_new { a.get_call_logs_json() } else { self.llm_adapter.get_call_logs_json() };
                    // Aggregate rollups
                    let mut total_cost = 0.0_f64;
                    ...
                    if let serde_json::Value::Array(arr) = &logs_val {
                        for entry in arr {
                            ...
                            let ms = entry.get("elapsed_ms").and_then(|v| v.as_u64()).unwrap_or(0);
                            total_cost += cost;
                            let e = by_model.entry(model).or_insert_with(|| serde_json::json!({"calls":0, "total_ms":0, "total_cost_usd":0.0}));
                            if let Some(obj) = e.as_object_mut() {
                                let calls = obj.get_mut("calls").and_then(|v| v.as_u64()).unwrap_or(0) + 1;
                                let tot_ms = obj.get_mut("total_ms").and_then(|v| v.as_u64()).unwrap_or(0) + ms;
                                let tot_cost = obj.get_mut("total_cost_usd").and_then(|v| v.as_f64()).unwrap_or(0.0) + cost;
                                obj.insert("calls".to_string(), serde_json::Value::from(calls));
                                obj.insert("total_ms".to_string(), serde_json::Value::from(tot_ms));
                                obj.insert("total_cost_usd".to_string(), serde_json::Value::from(tot_cost));
                            }
                        }
                    }
                    let json = serde_json::json!({
                    ...
                    std::fs::write(&path, serde_json::to_string_pretty(&json).unwrap_or_else(|_| json.to_string())).map_err(|e| ExecutorError::IoError(format!("write_metrics_json: {}", e)))?;
                    if let Some(res) = result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                } else if function == "get_env_or" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("get_env_or requires (name, default)".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("name must be string".to_string())) };
                    let def = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("default must be string".to_string())) };
                    let val = std::env::var(&name).unwrap_or(def);
                    if let Some(res)=result { self.store_value(res, RuntimeValue::String(val))?; }
                    Ok(())
                    ...
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                } else if function == "mcp__run_ws_server" || function == "mcp.run_ws_server" || function == "mcp__serve_ws_server" || function == "mcp.serve_ws_server" {
                    ...
                    if args.len()<1 { return Err(ExecutorError::ArgumentError("mcp.tool_call requires name and optional args".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("tool name must be string".to_string())) };
                    let json_args = if args.len()>=2 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::Json(j)=>j, RuntimeValue::String(s)=>serde_json::Value::String(s), _=>serde_json::Value::Null } } else { serde_json::Value::Null };
                    let out = self.execute_tool_call_json(&name, &json_args)?;
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(out))?; }
                    Ok(())
                } else if function == "mcp__set_quota" || function == "mcp.set_quota" {
                    if args.len()<2 { return Err(ExecutorError::ArgumentError("mcp.set_quota requires (tool, allowed_calls)".to_string())); }
                    let tool = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("tool must be string".to_string())) };
                    let allowed = match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=> i.max(0) as u64, RuntimeValue::Float(f)=> (f.max(0.0)) as u64, RuntimeValue::String(s)=> s.parse::<u64>().unwrap_or(0), _=>0 };
                    let meta = self.tool_registry.entry(tool).or_insert(ToolMeta{ allowed_calls: None, used_calls: 0, scope: None });
                    meta.allowed_calls = Some(allowed);
                    meta.used_calls = 0;
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                } else if function == "data_validate_schema" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("data_validate_schema requires (json, schema)".to_string())); }
                    let json = match self.evaluate_expression(args[0].clone())? {
                        RuntimeValue::Json(j)=>j,
                        RuntimeValue::String(s)=> serde_json::from_str(&s).map_err(|e| ExecutorError::JsonError(format!("parse json: {}", e)))?,
                        other=> return Err(ExecutorError::ArgumentError(format!("first arg must be JSON or string: {:?}", other)))
                    };
                    let schema = match self.evaluate_expression(args[1].clone())? {
                        RuntimeValue::Json(j)=>j,
                        RuntimeValue::String(s)=> serde_json::from_str(&s).map_err(|e| ExecutorError::JsonError(format!("parse schema: {}", e)))?,
                        other=> return Err(ExecutorError::ArgumentError(format!("second arg must be JSON schema or string: {:?}", other)))
                    };
                    let compiled = jsonschema::JSONSchema::compile(&schema).map_err(|e| ExecutorError::JsonError(format!("schema compile: {}", e)))?;
                    let valid = compiled.validate(&json).is_ok();
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(valid))?; }
                    Ok(())
                } else if function == "pii_redact" {
                    if args.len()!=1 { return Err(ExecutorError::ArgumentError("pii_redact requires text".to_string())); }
                    let text = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("text must be string".to_string())) };
                    ...
                    if let Some(res)=result { self.store_value(res, RuntimeValue::String(redacted))?; }
                    Ok(())
                } else if function == "memory_index__set_metadata" || function == "memory_index.set_metadata" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("set_metadata requires path, metadata".to_string())); }
                    let path = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("path must be string".to_string())) };
                    let metadata = match self.evaluate_expression(args[1].clone())? {
                        RuntimeValue::Json(j)=>j,
                    ...
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("data_validate_schema_file requires (json_path, schema_path)".to_string())); }
                    let jpath = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("json_path must be string".to_string())) };
                    let spath = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("schema_path must be string".to_string())) };
                    let jtxt = std::fs::read_to_string(&jpath).map_err(|e| ExecutorError::IoError(format!("read json: {}", e)))?;
                    let stxt = std::fs::read_to_string(&spath).map_err(|e| ExecutorError::IoError(format!("read schema: {}", e)))?;
                    let json: Value = serde_json::from_str(&jtxt).map_err(|e| ExecutorError::JsonError(format!("parse json: {}", e)))?;
                    let schema: Value = serde_json::from_str(&stxt).map_err(|e| ExecutorError::JsonError(format!("parse schema: {}", e)))?;
                    let compiled = jsonschema::JSONSchema::compile(&schema).map_err(|e| ExecutorError::JsonError(format!("schema compile: {}", e)))?;
                    let valid = compiled.validate(&json).is_ok();
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(valid))?; }
                    Ok(())
                } else if function == "rag_eval.precision_at_k" || function == "rag_eval__precision_at_k" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("precision_at_k requires (hits, gold)".to_string())); }
                    let hits = match self.evaluate_expression(args[0].clone())? { RuntimeValue::Json(Value::Array(a))=>a, _=>return Err(ExecutorError::ArgumentError("hits must be array".to_string())) };
                    let gold = match self.evaluate_expression(args[1].clone())? { RuntimeValue::Json(Value::Array(a))=> a.into_iter().filter_map(|v| v.as_str().map(|s| s.to_lowercase())).collect::<Vec<_>>(), _=>return Err(ExecutorError::ArgumentError("gold must be array of strings".to_string())) };
                    let k = hits.len().max(1) as f64;
                    let mut tp = 0.0;
                    for h in hits { let text = h.get("content").and_then(|v| v.as_str()).unwrap_or("").to_lowercase(); if gold.iter().any(|g| text.contains(g)) { tp+=1.0; } }
                    let p = tp / k;
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Float(p))?; }
                    Ok(())
                } else if function == "rag_eval.recall_at_k" || function == "rag_eval__recall_at_k" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("recall_at_k requires (hits, gold)".to_string())); }
                    let hits = match self.evaluate_expression(args[0].clone())? { RuntimeValue::Json(Value::Array(a))=>a, _=>return Err(ExecutorError::ArgumentError("hits must be array".to_string())) };
                    let gold = match self.evaluate_expression(args[1].clone())? { RuntimeValue::Json(Value::Array(a))=> a.into_iter().filter_map(|v| v.as_str().map(|s| s.to_lowercase())).collect::<Vec<_>>(), _=>return Err(ExecutorError::ArgumentError("gold must be array of strings".to_string())) };
                    let mut tp = 0.0;
                    for h in hits { let text = h.get("content").and_then(|v| v.as_str()).unwrap_or("").to_lowercase(); if gold.iter().any(|g| text.contains(g)) { tp+=1.0; } }
                    let r = tp / (gold.len().max(1) as f64);
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Float(r))?; }
                    Ok(())
                } else if function == "rag_eval.f1_at_k" || function == "rag_eval__f1_at_k" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("f1_at_k requires (hits, gold)".to_string())); }
                    let hits = match self.evaluate_expression(args[0].clone())? { RuntimeValue::Json(Value::Array(a))=>a, _=>return Err(ExecutorError::ArgumentError("hits must be array".to_string())) };
                    let gold = match self.evaluate_expression(args[1].clone())? { RuntimeValue::Json(Value::Array(a))=> a.into_iter().filter_map(|v| v.as_str().map(|s| s.to_lowercase())).collect::<Vec<_>>(), _=>return Err(ExecutorError::ArgumentError("gold must be array of strings".to_string())) };
                    let k = hits.len().max(1) as f64;
                    let mut tp = 0.0;
                    ...
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Float(f1))?; }
                    Ok(())
                    ...
                    if args.len()!=1 { return Err(ExecutorError::ArgumentError("load_parquet requires path".to_string())); }
                    let path = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("path must be string".to_string())) };
                    let ds = self.data_processor.load_parquet("parquet", &path)?;
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Dataset(Arc::new(ds)))?; }
                    Ok(())
                } else if function == "save_parquet" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("save_parquet requires dataset, path".to_string())); }
                    let ds = match self.evaluate_expression(args[0].clone())? { RuntimeValue::Dataset(d)=>d, _=>return Err(ExecutorError::ArgumentError("first arg must be dataset".to_string())) };
                    let path = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("path must be string".to_string())) };
                    ...
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("assert_true requires condition [and optional message]".to_string())); }
                    let cond = self.evaluate_expression(args[0].clone())?;
                    let ok = match cond { RuntimeValue::Boolean(b)=>b, RuntimeValue::Integer(i)=> i!=0, RuntimeValue::Float(f)=> f!=0.0, RuntimeValue::String(s)=> !s.is_empty(), _=> false };
                    if !ok {
                        let msg = if args.len()>=2 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, other=>format!("Assertion failed: {:?}", other) } } else { "Assertion failed".to_string() };
                        return Err(ExecutorError::RuntimeError(msg));
                    }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                } else if function == "assert_eq" {
                    if args.len()<2 { return Err(ExecutorError::ArgumentError("assert_eq requires two values [and optional message]".to_string())); }
                    let a = self.evaluate_expression(args[0].clone())?;
                    let b = self.evaluate_expression(args[1].clone())?;
                    let equal = format!("{:?}", a) == format!("{:?}", b);
                    if !equal {
                        let msg = if args.len()>=3 { match self.evaluate_expression(args[2].clone())? { RuntimeValue::String(s)=>s, _=>"".to_string() } } else { String::new() };
                        let base = format!("assert_eq failed: left={:?} right={:?}", a, b);
                        return Err(ExecutorError::RuntimeError(if msg.is_empty(){base}else{format!("{} - {}", base, msg)}));
                    }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                } else if function == "assert_snapshot" {
                    if args.len() < 2 { return Err(ExecutorError::ArgumentError("assert_snapshot requires name and value".to_string())); }
                    let name_val = self.evaluate_expression(args[0].clone())?;
                    let name = match name_val { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let value_val = self.evaluate_expression(args[1].clone())?;
                    let value_str = match value_val { RuntimeValue::String(s)=>s, RuntimeValue::Json(j)=>j.to_string(), other=>format!("{:?}", other) };
                    let path = if name.contains('/') || name.contains('\\') { name.clone() } else { format!("golden/{}.txt", name) };
                    let update = std::env::var("LEXON_SNAPSHOT_UPDATE").map(|v| v=="1"||v.eq_ignore_ascii_case("true")).unwrap_or(false);
                    match std::fs::read_to_string(&path) {
                        Ok(existing) => {
                            if existing != value_str {
                                if update {
                                    std::fs::write(&path, &value_str).map_err(|e| ExecutorError::RuntimeError(format!("snapshot update failed '{}': {}", path, e)))?;
                                } else {
                                    return Err(ExecutorError::RuntimeError(format!("assert_snapshot failed: {} does not match", path)));
                                }
                            }
                        }
                        Err(_) => {
                            if update {
                                std::fs::write(&path, &value_str).map_err(|e| ExecutorError::RuntimeError(format!("snapshot create failed '{}': {}", path, e)))?;
                            } else {
                                return Err(ExecutorError::RuntimeError(format!("snapshot missing: {} (set LEXON_SNAPSHOT_UPDATE=1 to create)", path)));
                            }
                        }
                    }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    ...
                    if args.len() < 2 { return Err(ExecutorError::ArgumentError("timeout requires ms and prompt [optional model]".to_string())); }
                    let ms_value = self.evaluate_expression(args[0].clone())?;
                    let ms = match ms_value { RuntimeValue::Integer(i)=>i as u64, RuntimeValue::Float(f)=>f as u64, RuntimeValue::String(s)=> s.parse::<u64>().unwrap_or(0), _=>0 };
                    let prompt_value = self.evaluate_expression(args[1].clone())?;
                    let prompt = match prompt_value { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let model_opt = if args.len() > 2 {
                        match self.evaluate_expression(args[2].clone())? { RuntimeValue::String(s)=>Some(s), _=>None }
                    } else { self.config.llm_model.clone() };
                    ...
                    let response = tokio::task::block_in_place(|| {
                        tokio::runtime::Handle::current().block_on(async {
                            match tokio::time::timeout(std::time::Duration::from_millis(ms), fut).await {
                                Ok(Ok(r)) => Ok(r),
                                Ok(Err(e)) => Err(e),
                                Err(_) => Err(ExecutorError::RuntimeError(format!("timeout exceeded: {} ms", ms)))
                            }
                        })
                    })?;
                    ...
                    if args.len() < 3 { return Err(ExecutorError::ArgumentError("retry requires attempts, backoff_ms, prompt [optional model]".to_string())); }
                    let attempts_value = self.evaluate_expression(args[0].clone())?;
                    let backoff_value = self.evaluate_expression(args[1].clone())?;
                    let attempts = match attempts_value { RuntimeValue::Integer(i)=>i as u32, RuntimeValue::Float(f)=>f as u32, RuntimeValue::String(s)=> s.parse::<u32>().unwrap_or(1), _=>1 };
                    let backoff_ms = match backoff_value { RuntimeValue::Integer(i)=>i as u64, RuntimeValue::Float(f)=>f as u64, RuntimeValue::String(s)=> s.parse::<u64>().unwrap_or(0), _=>0 };
                    let prompt_value = self.evaluate_expression(args[2].clone())?;
                    let prompt = match prompt_value { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let model_opt = if args.len() > 3 {
                        match self.evaluate_expression(args[3].clone())? { RuntimeValue::String(s)=>Some(s), _=>None }
                    } else { self.config.llm_model.clone() };
                    ...
                    for attempt in 1..=attempts {
                        ...
                        match res {
                            Ok(response) => {
                                if let Some(res_ref)=result { self.store_value(res_ref, RuntimeValue::String(response))?; }
                                return Ok(());
                            }
                            Err(e) => {
                                last_err = Some(e);
                                if attempt < attempts { std::thread::sleep(std::time::Duration::from_millis(backoff_ms)); }
                            }
                        }
                    }
                    Err(last_err.unwrap_or_else(|| ExecutorError::RuntimeError("retry failed".to_string())))
                } else if function == "configure_validation" {
                    if args.is_empty() { if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; } return Ok(()); }
                    let cfg = match self.evaluate_expression(args[0].clone())? { RuntimeValue::Json(j)=>Some(j), RuntimeValue::String(s)=>serde_json::from_str(&s).ok(), _=>None };
                    if cfg.is_none() { if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; } return Ok(()); }
                    let cfg = cfg.unwrap();
                    if let Some(mc)=cfg.get("min_confidence").and_then(|v| v.as_f64()) { self.validation_min_confidence = mc; }
                    if let Some(ma)=cfg.get("max_attempts").and_then(|v| v.as_u64()) { self.validation_max_attempts = ma as u32; }
                    if let Some(vt)=cfg.get("validation_types").and_then(|v| v.as_array()).and_then(|a| a.get(0)).and_then(|v| v.as_str()) { self.validation_strategy = vt.to_string(); }
                    if let Some(dom)=cfg.get("domain").and_then(|v| v.as_str()) { self.validation_domain = Some(dom.to_string()); }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Boolean(true))?; }
                    Ok(())
                } else if function == "ask_with_validation" {
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("ask_with_validation requires (prompt, [config])".to_string())); }
                    ...
                    let cfg = if args.len()>=2 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::Json(j)=>Some(j), RuntimeValue::String(s)=>serde_json::from_str(&s).ok(), _=>None } } else { None };
                    use crate::executor::llm_adapter::{AntiHallucinationConfig, ValidationStrategy};
                    let min_conf = cfg.as_ref().and_then(|j| j.get("min_confidence").and_then(|v| v.as_f64())).unwrap_or(self.validation_min_confidence);
                    let max_att = cfg.as_ref().and_then(|j| j.get("max_attempts").and_then(|v| v.as_u64())).map(|v| v as usize).unwrap_or(self.validation_max_attempts as usize);
                    let strat_str = cfg.as_ref().and_then(|j| j.get("validation_types").and_then(|v| v.as_array()).and_then(|a| a.get(0)).and_then(|v| v.as_str())).unwrap_or(&self.validation_strategy);
                    ...
                    let config = AntiHallucinationConfig { validation_strategy: strategy, confidence_threshold: min_conf, max_validation_attempts: max_att, use_fact_checking: true, cross_reference_models: vec![] };
                    let validation_result = tokio::task::block_in_place(|| { tokio::runtime::Handle::current().block_on(async { self.llm_adapter.ask_safe(&prompt, None, Some(config)).await }) })?;
                    if let Some(res)=result { self.store_value(res, RuntimeValue::String(validation_result.validated_content))?; }
                    Ok(())
                    ...
                    Ok(())
                } else if function == "memory_index.ingest_chunks" || function == "memory_index__ingest_chunks" {
                    // memory_index.ingest_chunks(paths, options_json?) -> int
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("memory_index.ingest_chunks requires at least 1 argument: paths".to_string())); }
                    let paths_value = self.evaluate_expression(args[0].clone())?;
                    ...
                    match paths_value {
                        ...
                        RuntimeValue::Json(Value::Array(arr)) => {
                            for v in arr.iter() {
                                if let Some(s) = v.as_str() { paths.push(s.to_string()); } else {
                                    return Err(ExecutorError::ArgumentError("memory_index.ingest_chunks paths array must contain only strings".to_string()));
                                }
                            }
                        }
                        _ => return Err(ExecutorError::ArgumentError("memory_index.ingest_chunks paths must be a string or array".to_string())),
                    }
                    ...
                    if args.len() > 1 {
                        ...
                        match options_val {
                            RuntimeValue::Json(obj) => {
                                if let Some(b) = obj.get("by").and_then(|v| v.as_str()) { by = b.to_string(); }
                                if let Some(sz) = obj.get("size").and_then(|v| v.as_u64()) { size = sz as usize; }
                                if let Some(ov) = obj.get("overlap").and_then(|v| v.as_u64()) { overlap = ov as usize; }
                            }
                            RuntimeValue::String(s) => {
                                // Try JSON first
                                if let Ok(obj) = serde_json::from_str::<serde_json::Value>(&s) {
                                    ...
                                    for part in t.split(',') {
                                        if let Some((k,v)) = part.split_once(':') {
                                            ...
                                            match key {
                                                "by" => by = val.to_string(),
                                                "size" => if let Ok(n)=val.parse::<usize>() { size = n; },
                                                "overlap" => if let Ok(n)=val.parse::<usize>() { overlap = n; },
                                                _ => {}
                                            }
                                        }
                                    }
                    ...
                    if let Some(ref mut vm) = self.vector_memory_system {
                        println!("🧠 memory_index.ingest_chunks: by={}, size={}, overlap={}", by, size, overlap);
                        let count = vm.ingest_documents_chunks(&paths, &by, size, overlap)?;
                        if let Some(res) = result { self.store_value(res, RuntimeValue::Integer(count as i64))?; }
                        Ok(())
                    } else {
                        Err(ExecutorError::RuntimeError("Vector memory system not initialized".to_string()))
                    }
                } else if function == "rag.chunk" || function == "rag__chunk" {
                    // rag.chunk(text, options_json?) -> [string]
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("rag.chunk requires at least 1 argument: text".to_string())); }
                    let text_val = self.evaluate_expression(args[0].clone())?;
                    let text = match text_val { RuntimeValue::String(s) => s, _ => return Err(ExecutorError::ArgumentError("rag.chunk text must be a string".to_string())) };
                    let mut by = "tokens".to_string();
                    let mut size: usize = 200;
                    let mut overlap: usize = 40;
                    if args.len() > 1 {
                    ...
                    let chunks_json = if let Some(ref vm) = self.vector_memory_system {
                        ...
                        serde_json::Value::Array(chunks.into_iter().map(|s| serde_json::Value::String(s)).collect())
                    } else {
                        let tokens: Vec<&str> = text.split_whitespace().collect();
                        let mut out: Vec<String> = Vec::new();
                        let mut i=0usize; while i<tokens.len(){ let end=(i+size).min(tokens.len()); out.push(tokens[i..end].join(" ")); if end==tokens.len(){break;} let step=size.saturating_sub(overlap).max(1); i+=step; }
                        serde_json::Value::Array(out.into_iter().map(|s| serde_json::Value::String(s)).collect())
                    };
                    if let Some(res) = result { self.store_value(res, RuntimeValue::Json(chunks_json))?; }
                    Ok(())
                } else if function == "rag.tokenize" || function == "rag__tokenize" {
                    // rag.tokenize(text, model?) -> [string]
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("rag.tokenize requires at least 1 argument: text".to_string())); }
                    let text = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s) => s, _ => return Err(ExecutorError::ArgumentError("rag.tokenize text must be a string".to_string())) };
                    let model = if args.len()>1 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>Some(s), _=>None } } else { None };
                    let toks_json: Vec<serde_json::Value> = if let Some((bpe, ids)) = Self::encode_tokens_precise(&text, model.as_deref()) {
                        Self::tokens_to_strings(&bpe, &ids).into_iter().map(|s| serde_json::Value::String(s)).collect()
                    } else {
                        text.split_whitespace().map(|t| serde_json::Value::String(t.to_string())).collect()
                    };
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(serde_json::Value::Array(toks_json)))?; }
                    Ok(())
                } else if function == "rag.token_count" || function == "rag__token_count" {
                    // rag.token_count(text, model?) -> int
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("rag.token_count requires at least 1 argument: text".to_string())); }
                    let text = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s) => s, _ => return Err(ExecutorError::ArgumentError("rag.token_count text must be a string".to_string())) };
                    let model = if args.len()>1 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>Some(s), _=>None } } else { None };
                    let count = if let Some((_, ids)) = Self::encode_tokens_precise(&text, model.as_deref()) { ids.len() as i64 } else { text.split_whitespace().count() as i64 };
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Integer(count))?; }
                    Ok(())
                } else if function == "rag.chunk_tokens" || function == "rag__chunk_tokens" {
                    // rag.chunk_tokens(text, size, overlap?, model?) -> [string]
                    if args.len()<2 { return Err(ExecutorError::ArgumentError("rag.chunk_tokens requires text and size".to_string())); }
                    let text = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s) => s, _ => return Err(ExecutorError::ArgumentError("rag.chunk_tokens text must be a string".to_string())) };
                    let size = match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i) => i as usize, _ => return Err(ExecutorError::ArgumentError("rag.chunk_tokens size must be an integer".to_string())) };
                    let overlap = if args.len()>2 { match self.evaluate_expression(args[2].clone())? { RuntimeValue::Integer(i)=>i as usize, _=>0 } } else { 0 };
                    let model = if args.len()>3 { match self.evaluate_expression(args[3].clone())? { RuntimeValue::String(s)=>Some(s), _=>None } } else { None };
                    ...
                    if let Some((bpe, ids)) = Self::encode_tokens_precise(&text, model.as_deref()) {
                        ...
                        let mut i=0usize; while i<toks.len(){ let end=(i+size).min(toks.len()); out.push(serde_json::Value::String(toks[i..end].join(" "))); if end==toks.len(){break;} let step=size.saturating_sub(overlap).max(1); i+=step; }
                    }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(serde_json::Value::Array(out)))?; }
                    Ok(())
                } else if function == "rag.rerank" || function == "rag__rerank" {
                    // rag.rerank(results_json, query) -> results_json
                    if args.len()<2 { return Err(ExecutorError::ArgumentError("rag.rerank requires results and query".to_string())); }
                    let resv = self.evaluate_expression(args[0].clone())?;
                    let query = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, _=> return Err(ExecutorError::ArgumentError("rag.rerank query must be a string".to_string())) };
                    let mut arr: Vec<serde_json::Value> = match resv { RuntimeValue::Json(serde_json::Value::Array(a))=>a, _=>Vec::new() };
                    let ql = query.to_lowercase();
                    arr.sort_by(|a,b| {
                        let ascore = a.get("hybrid_score").and_then(|v| v.as_f64()).unwrap_or_else(||{
                            let c=a.get("content").and_then(|v| v.as_str()).unwrap_or("").to_lowercase();
                            ql.split_whitespace().filter(|w| c.contains(*w)).count() as f64
                        });
                        let bscore = b.get("hybrid_score").and_then(|v| v.as_f64()).unwrap_or_else(||{
                            let c=b.get("content").and_then(|v| v.as_str()).unwrap_or("").to_lowercase();
                            ql.split_whitespace().filter(|w| c.contains(*w)).count() as f64
                        });
                    ...
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("rag.fuse_passages requires results".to_string())); }
                    let resv = self.evaluate_expression(args[0].clone())?;
                    let maxp = if args.len()>1 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=>i as usize, _=> 5 } } else { 5 };
                    // MMR-like fusion to reduce redundancy
                    let mut selected: Vec<String> = Vec::new();
                    if let RuntimeValue::Json(serde_json::Value::Array(a)) = resv {
                        let mut cands: Vec<String> = a.iter().filter_map(|it| it.get("content").and_then(|v| v.as_str()).map(|s| s.to_string())).collect();
                        while !cands.is_empty() && selected.len()<maxp {
                            // pick best by coverage - redundancy
                            let mut best_idx = 0usize; let mut best_score = f32::MIN;
                            for (idx,c) in cands.iter().enumerate() {
                                let coverage = c.len() as f32;
                                let redundancy = selected.iter().map(|s| Self::jaccard_sim_local(s,c)).fold(0f32, |acc,x| acc.max(x));
                                let lambda = 0.7f32;
                    ...
                    let mut out = String::new();
                    for (i,s) in selected.iter().enumerate() { if i>0 { out.push_str("\n\n"); } out.push_str(s); }
                    if let Some(res)=result { self.store_value(res, RuntimeValue::String(out))?; }
                    Ok(())
                } else if function == "rag.fuse_passages_semantic" || function == "rag__fuse_passages_semantic" {
                    // rag.fuse_passages_semantic(results_json, max?, lambda?, citations?) -> string
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("rag.fuse_passages_semantic requires results".to_string())); }
                    let resv = self.evaluate_expression(args[0].clone())?;
                    let maxp = if args.len()>1 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=>i as usize, _=> 5 } } else { 5 };
                    let lambda = if args.len()>2 { match self.evaluate_expression(args[2].clone())? { RuntimeValue::Float(f)=>f as f32, RuntimeValue::Integer(i)=> i as f32, _=> 0.7 } } else { 0.7 };
                    let with_citations = if args.len()>3 { match self.evaluate_expression(args[3].clone())? { RuntimeValue::Boolean(b)=>b, _=> false } } else { false };
                    let mut selected: Vec<String> = Vec::new();
                    ...
                    if let RuntimeValue::Json(serde_json::Value::Array(a)) = resv {
                        for it in a.iter() {
                            if let Some(s) = it.get("content").and_then(|v| v.as_str()) { cands.push(s.to_string()); cands_meta.push(it.clone()); }
                        }
                    }
                    if cands.is_empty() { if let Some(res)=result { self.store_value(res, RuntimeValue::String(String::new()))?; } return Ok(()); }
                    ...
                    let have_embs = if let Some(ref vm) = self.vector_memory_system {
                        for s in cands.iter() { emb_map.insert(s.clone(), vm.embed_text(s)); }
                        true
                    } else { false };
                    while !cands.is_empty() && selected.len()<maxp {
                        let mut best_idx = 0usize; let mut best_score = f32::MIN;
                        for (idx,c) in cands.iter().enumerate() {
                            ...
                            let redundancy = if have_embs && !selected.is_empty() {
                                ...
                                let e_c: &[f32] = match emb_map.get(c) { Some(v) => v.as_slice(), None => { let empty: &[f32] = &[]; empty } };
                                let mut max_sim = 0.0f32;
                                for s in selected.iter() {
                                    if let Some(e_s) = emb_map.get(s) { let sim = Self::cosine_sim_local(e_c, e_s); if sim>max_sim { max_sim = sim; } }
                                }
                                max_sim
                            } else {
                                selected.iter().map(|s| Self::jaccard_sim_local(s,c)).fold(0f32, |acc,x| acc.max(x))
                            };
                            let score = (lambda*coverage) - ((1.0-lambda)*redundancy);
                            if score>best_score { best_score=score; best_idx=idx; }
                        }
                    ...
                    let mut out = String::new();
                    for (i,s) in selected.iter().enumerate() { if i>0 { out.push_str("\n\n"); } if with_citations { out.push_str(&format!("[{}] ", i+1)); } out.push_str(s); }
                    if with_citations {
                        out.push_str("\n\nCitations:\n");
                    ...
                    if args.len()<2 { return Err(ExecutorError::ArgumentError("rag.rerank_llm requires results and query".to_string())); }
                    let resv = self.evaluate_expression(args[0].clone())?;
                    let query = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, _=> return Err(ExecutorError::ArgumentError("rag.rerank_llm query must be a string".to_string())) };
                    let model = if args.len()>2 { match self.evaluate_expression(args[2].clone())? { RuntimeValue::String(s)=>Some(s), _=>None } } else { None };
                    let top_k = if args.len()>3 { match self.evaluate_expression(args[3].clone())? { RuntimeValue::Integer(i)=>i as usize, _=>10 } } else { 10 };
                    let mut items: Vec<serde_json::Value> = match resv { RuntimeValue::Json(serde_json::Value::Array(a))=>a, _=>Vec::new() };
                    ...
                    for it in items.drain(..) {
                        ...
                        let resp = if let Some(ref mut la) = self.llm_adapter_new {
                            tokio::task::block_in_place(|| tokio::runtime::Handle::current().block_on(async {
                                la.call_llm_async(model.as_deref(), Some(0.0), Some("You are a precise scorer."), Some(&prompt), None, None, &HashMap::new()).await
                            }))?
                        } else {
                            self.llm_adapter.call_llm(model.as_deref(), Some(0.0), Some("You are a precise scorer."), Some(&prompt), None, None, &HashMap::new())?
                        };
                        let score = Self::parse_score_local(&resp);
                        scored.push((score, it));
                    }
                    scored.sort_by(|a,b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
                    scored.truncate(top_k);
                    let out = serde_json::Value::Array(scored.into_iter().map(|(_,v)| v).collect());
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Json(out))?; }
                    Ok(())
                } else if function == "rag.rerank_cross_encoder" || function == "rag__rerank_cross_encoder" {
                    // rag.rerank_cross_encoder(results_json, query, model?, top_k?, batch_size?) -> results_json
                    if args.len()<2 { return Err(ExecutorError::ArgumentError("rag.rerank_cross_encoder requires results and query".to_string())); }
                    let resv = self.evaluate_expression(args[0].clone())?;
                    let query = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, _=> return Err(ExecutorError::ArgumentError("rag.rerank_cross_encoder query must be a string".to_string())) };
                    let model = if args.len()>2 { match self.evaluate_expression(args[2].clone())? { RuntimeValue::String(s)=>Some(s), _=>None } } else { None };
                    let top_k = if args.len()>3 { match self.evaluate_expression(args[3].clone())? { RuntimeValue::Integer(i)=>i as usize, _=>10 } } else { 10 };
                    let env_batch = std::env::var("LEXON_RERANK_BATCH_SIZE").ok().and_then(|s| s.parse::<usize>().ok()).unwrap_or(8);
                    let batch_size = if args.len()>4 { match self.evaluate_expression(args[4].clone())? { RuntimeValue::Integer(i)=> (i as usize).max(1), _=> env_batch } } else { env_batch };
                    let mut items: Vec<serde_json::Value> = match resv { RuntimeValue::Json(serde_json::Value::Array(a))=>a, _=>Vec::new() };
                    if let Some(max_items) = std::env::var("LEXON_RERANK_MAX_ITEMS").ok().and_then(|s| s.parse::<usize>().ok()) { if items.len() > max_items { items.truncate(max_items); } }
                    ...
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("rag.summarize_chunks requires chunks".to_string())); }
                    let resv = self.evaluate_expression(args[0].clone())?;
                    let model = if args.len()>1 { match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>Some(s), _=>None } } else { None };
                    let mut joined = String::new();
                    if let RuntimeValue::Json(serde_json::Value::Array(a)) = resv { for s in a { if let Some(t)=s.as_str() { joined.push_str("- "); joined.push_str(t); joined.push('\n'); } } }
                    // Use LLM adapter to summarize
                    let prompt = format!("Summarize the following passages succinctly in 3-4 bullet points:\n\n{}", joined);
                    let sum = if let Some(ref mut la) = self.llm_adapter_new {
                        tokio::task::block_in_place(|| tokio::runtime::Handle::current().block_on(async {
                            la.call_llm_async(model.as_deref(), Some(0.5), Some("You are a concise summarizer."), Some(&prompt), None, None, &HashMap::new()).await
                        }))?
                    } else {
                        // legacy path
                        self.llm_adapter.call_llm(Some("simulated"), Some(0.5), Some("You are a concise summarizer."), Some(&prompt), None, None, &HashMap::new())?
                    };
                    if let Some(res)=result { self.store_value(res, RuntimeValue::String(sum))?; }
                    Ok(())
                    ...
                    if args.len()<2 { return Err(ExecutorError::ArgumentError("rag.optimize_window requires results and target_tokens".to_string())); }
                    let resv = self.evaluate_expression(args[0].clone())?;
                    let target = match self.evaluate_expression(args[1].clone())? { RuntimeValue::Integer(i)=> i as usize, _=> return Err(ExecutorError::ArgumentError("rag.optimize_window target_tokens must be int".to_string())) };
                    let model = if args.len()>2 { match self.evaluate_expression(args[2].clone())? { RuntimeValue::String(s)=>Some(s), _=>None } } else { None };
                    let margin = if args.len()>3 { match self.evaluate_expression(args[3].clone())? { RuntimeValue::Integer(i)=> i as usize, _=> 256 } } else { 256 };
                    let mut items: Vec<serde_json::Value> = match resv { RuntimeValue::Json(serde_json::Value::Array(a))=>a, _=>Vec::new() };
                    ...
                    for it in items.drain(..) {
                        let content = it.get("content").and_then(|v| v.as_str()).unwrap_or("");
                        let score = it.get("hybrid_score").and_then(|v| v.as_f64()).unwrap_or(0.5) as f32;
                        let tokens = if let Some((_bpe, ids)) = Self::encode_tokens_precise(content, model.as_deref()) { ids.len() } else { Self::estimate_token_count_local(content, model.as_deref()) };
                        scored.push((score, tokens, it));
                    }
                    // Greedy by score/tokens
                    scored.sort_by(|a,b| (b.0/b.1.max(1) as f32).partial_cmp(&(a.0/a.1.max(1) as f32)).unwrap_or(std::cmp::Ordering::Equal));
                    let mut budget = target.saturating_sub(margin);
                    let mut selected: Vec<serde_json::Value> = Vec::new();
                    for (s,t,it) in scored.into_iter() {
                    ...
                    if args.len()!=3 { return Err(ExecutorError::ArgumentError("context_retrieve requires session_id, query, k".to_string())); }
                    let session_id = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("session_id must be string".to_string())) };
                    let query = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("query must be string".to_string())) };
                    let k = match self.evaluate_expression(args[2].clone())? { RuntimeValue::Integer(i)=>i as usize, _=>return Err(ExecutorError::ArgumentError("k must be integer".to_string())) };
                    let history = self.memory_manager.get_session_history(&session_id).unwrap_or_default();
                    ...
                    if let Some(res)=result { self.store_value(res, RuntimeValue::String(excerpt))?; }
                    Ok(())
                } else if function == "context_merge" {
                    if args.is_empty() { return Err(ExecutorError::ArgumentError("context_merge requires [session_ids]".to_string())); }
                    let session_ids: Vec<String> = match self.evaluate_expression(args[0].clone())? {
                        RuntimeValue::Json(Value::Array(arr)) => arr.into_iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect(),
                        RuntimeValue::String(s)=>vec![s],
                    ...
                    if let Some(res)=result { self.store_value(res, RuntimeValue::String(merged))?; }
                    Ok(())
                    ...
                    if binary_file.size > max_bytes {
                        return Err(ExecutorError::RuntimeError(format!("File exceeds limit: {} bytes > {}", binary_file.size, max_bytes)));
                    }
                        ...
                        match std::fs::write(&path, &binary_file.content) {
                            ...
                            Err(e) => {
                                if attempts >= max_attempts {
                                    return Err(ExecutorError::DataError(format!("Failed to save file {}: {}", path, e)));
                                }
                                attempts += 1;
                    ...
                    if args.len() < 2 { return Err(ExecutorError::ArgumentError("prompt.render requires (name, vars, [version])".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                            ...
                            match serde_json::from_str(&s) {
                                ...
                                Err(_) => {
                                    ...
                                    if trimmed.contains('=') {
                                        ...
                                        let t2 = trimmed.trim_start_matches('{').trim_end_matches('}');
                    ...
                    let ver = if args.len() > 2 { match self.evaluate_expression(args[2].clone())? { RuntimeValue::Integer(i)=>Some(i as i64), RuntimeValue::String(s)=>s.parse::<i64>().ok(), _=>None } } else { None };
                    let map = Self::load_prompts();
                    let entry = map.get(&name).and_then(|v| v.get("versions")).and_then(|v| v.as_array()).ok_or_else(|| ExecutorError::DataError("prompt not found".to_string()))?;
                    let pick = if let Some(v) = ver {
                        entry.iter().find(|e| e.get("version").and_then(|x| x.as_i64()) == Some(v))
                    } else {
                        entry.iter().max_by_key(|e| e.get("version").and_then(|x| x.as_i64()).unwrap_or(0))
                    }.ok_or_else(|| ExecutorError::DataError("prompt version not found".to_string()))?;
                    let content = pick.get("content").and_then(|v| v.as_str()).unwrap_or("");
                    let rendered = Self::render_template(content, &vars);
                    if let Some(res)=result { self.store_value(res, RuntimeValue::String(rendered))?; }
                    Ok(())
                } else if function == "prompt.render_kv" || function == "prompt__render_kv" {
                    // prompt.render_kv(name, key, value, [version])
                    if args.len() < 3 { return Err(ExecutorError::ArgumentError("prompt.render_kv requires (name, key, value, [version])".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let key = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let val = match self.evaluate_expression(args[2].clone())? { RuntimeValue::String(s)=>s, other=>format!("{:?}", other) };
                    let ver = if args.len() > 3 { match self.evaluate_expression(args[3].clone())? { RuntimeValue::Integer(i)=>Some(i as i64), RuntimeValue::String(s)=>s.parse::<i64>().ok(), _=>None } } else { None };
                    let map = Self::load_prompts();
                    let entry = map.get(&name).and_then(|v| v.get("versions")).and_then(|v| v.as_array()).ok_or_else(|| ExecutorError::DataError("prompt not found".to_string()))?;
                    let pick = if let Some(v) = ver { entry.iter().find(|e| e.get("version").and_then(|x| x.as_i64()) == Some(v)) } else { entry.iter().max_by_key(|e| e.get("version").and_then(|x| x.as_i64()).unwrap_or(0)) }.ok_or_else(|| ExecutorError::DataError("prompt version not found".to_string()))?;
                    let mut content = pick.get("content").and_then(|v| v.as_str()).unwrap_or("").to_string();
                    let needle = format!("{{{{{}}}}}", key);
                    content = content.replace(&needle, &val);
                    if let Some(res)=result { self.store_value(res, RuntimeValue::String(content))?; }
                    Ok(())
                } else if function == "eval.bleu" || function == "eval__bleu" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("eval.bleu requires (hypothesis, reference)".to_string())); }
                    let hyp = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("hypothesis must be string".to_string())) };
                    let refx = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("reference must be string".to_string())) };
                    // Simple BLEU-1 (unigram precision with clipping)
                    use std::collections::HashMap as Hm;
                    let mut ref_counts = Hm::new();
                    for w in refx.split_whitespace() { *ref_counts.entry(w).or_insert(0u32) += 1; }
                    let mut match_count = 0u32; let mut total = 0u32; let mut seen = Hm::new();
                    for w in hyp.split_whitespace() { total+=1; let c = seen.entry(w).or_insert(0u32); *c+=1; let allowed = *ref_counts.get(w).unwrap_or(&0); if *c <= allowed { match_count+=1; } }
                    let p = if total==0 { 0.0 } else { (match_count as f64)/(total as f64) };
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Float(p))?; }
                    Ok(())
                } else if function == "eval.rouge_l" || function == "eval__rouge_l" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("eval.rouge_l requires (hypothesis, reference)".to_string())); }
                    let hyp = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("hypothesis must be string".to_string())) };
                    let refx = match self.evaluate_expression(args[1].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("reference must be string".to_string())) };
                    ...
                    for i in 1..=a.len() { for j in 1..=b.len() { dp[i][j] = if a[i-1]==b[j-1] { dp[i-1][j-1]+1 } else { dp[i-1][j].max(dp[i][j-1]) }; } }
                    let lcs = dp[a.len()][b.len()] as f64;
                    let r = if b.is_empty() { 0.0 } else { lcs / (b.len() as f64) };
                    let p = if a.is_empty() { 0.0 } else { lcs / (a.len() as f64) };
                    let f = if (p+r) == 0.0 { 0.0 } else { (2.0*p*r)/(p+r) };
                    if let Some(res)=result { self.store_value(res, RuntimeValue::Float(f))?; }
                    Ok(())
                } else if function == "get_env_or" {
                    if args.len()!=2 { return Err(ExecutorError::ArgumentError("get_env_or requires (name, default)".to_string())); }
                    let name = match self.evaluate_expression(args[0].clone())? { RuntimeValue::String(s)=>s, _=>return Err(ExecutorError::ArgumentError("name must be string".to_string())) };
            ...
            LexInstruction::AsyncAsk {
                ...
                let mut llm_adapter = if let Some(ref a) = self.llm_adapter_new { a.clone() } else { self.llm_adapter.clone() };
