...
fn main() {
    ...
    // Handle REPL mode
    if args.repl {
        println!("🚀 LEXON Interactive REPL v1.0");
    ...
    match result {
        Ok(Some(exec_time)) => {
            if args.verbose {
                println!("✅ Execution completed in {:.2}ms", exec_time);
            }
        }
        Ok(None) => {
            if args.verbose {
...
fn compile_and_run(args: &Cli, _tokio_rt: &TokioRuntime) -> Result<Option<f64>, String> {
    ...
    if let Some(ir_path) = &args.load_ir {
        ...
        if args.verbose {
            println!("✅ HIR built successfully");
            println!("🔍 DEBUG: HIR nodes: {:?}", hir);
        }
        ...
        if args.verbose {
            println!("✅ LexIR generated successfully");
        }
    }
    ...
    if let Some(export_path) = &args.output_ir {
        ...
        if args.verbose {
            println!("✅ IR exported to {}", export_path.display());
        }
    }

    // --- 4. Check mode: validate and exit ---
    if args.check_only {
        println!("✅ Syntax and semantic checks passed");
        return Ok(None);
    }
...
fn run_repl() {
    ...
    println!("📖 LEXON Interactive REPL");
    println!("Commands:");
            ...
            Ok(_) => {
                ...
                match input {
                    ...
                    _ => {
                        ...
                        println!("🔧 REPL execution not yet implemented");
                        println!("💡 Use 'lexc file.lx' to run LEXON programs");
                    }
                }
            }
            Err(e) => {
...
fn check_syntax(args: &Cli, _tokio_rt: &TokioRuntime) -> Result<Option<f64>, String> {
    ...
    if args.verbose {
        println!("🔍 Checking syntax: {}", input_file.display());
    }
    ...
    if args.verbose {
        println!("✅ Syntax check passed ({} HIR nodes)", hir.len());
    }

    // Run linter if --lint flag is provided
    if args.lint {
        if args.verbose {
            println!("🔍 Running linter analysis...");
        }
...
fn run_benchmark(args: &Cli, _tokio_rt: &TokioRuntime) -> Result<Option<f64>, String> {
    ...
    println!("🏃 Running benchmark: {}", input_file.display());
...
fn run_linter(args: &Cli, _tokio_rt: &TokioRuntime) -> Result<Option<f64>, String> {
    ...
    if args.verbose {
        println!("🔍 Running linter on: {}", input_file.display());
    }
    ...
    if args.verbose {
        println!("✅ HIR built successfully ({} nodes)", hir_nodes.len());
    }
